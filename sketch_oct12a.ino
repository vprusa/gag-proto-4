...
// ================= Drawing: wrist-rotating skeleton =================
static void drawHandPivotModel() {
  ...
  // Wrist pivot centered (x slightly left for label room), red dot
  const int wristX = W/2 - 10;
  const int wristY = H/2 + 0;
  tft.fillCircle(wristX, wristY, 4, COL_WRIST);

  // Wrist yaw defines a global rotation of the entire skeleton
  const float yawW = yaw_[PART_TO_SENSOR[WRIST]];

  // Layout parameters
  const float baseAngle      = -40.0f; // local start angle for thumb ray
  const float stepAngle      =  20.0f; // spacing between finger rays
  const float guideLen       =  50.0f; // wrist->finger pivot
  const float fingerLen      =  40.0f; // finger line length
  ...
  for (int i=0; i<5; ++i) {
    ...
    // 2) Finger line orientation
    const uint8_t sIdx = PART_TO_SENSOR[i+1]; // THUMB..LITTLE
    float fingerYaw = yaw_[sIdx];
    float fingerLocalYaw = WRIST_RELATIVE ? (fingerYaw - yawW) : fingerYaw;
    if (fingerLocalYaw > 180.0f)  fingerLocalYaw -= 360.0f;
    if (fingerLocalYaw < -180.0f) fingerLocalYaw += 360.0f;

    // World finger angle = skeleton rotation + base finger dir + local finger yaw
  ...
  tft.setTextFont(1);
  tft.drawString(WRIST_RELATIVE ? "Skeleton: wrist-rot, fingers rel" : "Skeleton: wrist-rot, fingers abs",
                 W - 5, 10, 1);
}
